...
describe('空状態の組み立て', () => {
  ...
  it('閲覧履歴が無ければセクションを出さない', async () => {
    expect((await buildBootstrap('modal', NOW)).sections).toEqual([]);
  });
  ...
  it('最近開いた項目が新しい順に並ぶ', async () => {
    ...
    const [section] = (await buildBootstrap('modal', NOW)).sections;
  ...
  it('先頭の行が選択済みで、Enter のヒントが付く', async () => {
    ...
    const rows = (await buildBootstrap('modal', NOW)).sections[0]?.rows ?? [];
  ...
  it('件名が課題キーだけのときは等幅の識別子として出す', async () => {
    ...
    expect((await buildBootstrap('modal', NOW)).sections[0]?.rows[0]?.code).toBe('PROJ-9');
  });
  ...
  it('学習をオフにすると「学習で並び替え」を表示しない', async () => {
    ...
    expect((await buildBootstrap('modal', NOW)).sections[0]?.meta).toBeUndefined();
  });
});
