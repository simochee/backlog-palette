...
## M1. UI を Storybook で作る — 完了（2026-09-13、PR #4）
...
| | |
|---|---|
| 成果物 | API キー接続の導線（発行ページ + メモ欄 + 貼り付けバー、TanStack Form。OAuth は作らない、決定の記録 2026-09-13）、`backlog-js` のラッパーと Pacer のレート制御（スペース × 枠、storage で共有）、`rateLimit` による初期化、TanStack Query の `queryOptions` カタログと persister、マスタの先読み（プロジェクト・ステータス）、担当課題の取得、検索（**1 スペースの中**で `useQueries` により種別（課題・Wiki・ドキュメント）を並列、D-20。合流と保留は純粋関数、パネルの条件を `statusId[]` 等へ展開）、スペースの切り替え（別スペースの鍵とクライアントに切り替える）、カスタムドメインの動的登録 |
| 完了条件 | 偽スペースの E2E で「未接続 → 接続 → 空状態に担当課題が出る」「認証切れ（401）で結果の代わりに再接続行が出てパレットは閉じない」「429 で該当スペースの検索だけが待ちになり候補は動く」「根で別スペースを選ぶと切り替わる」が通る。ユニットで「『完了を除く』はプロジェクトごとの statusId 集合に展開される」 |
| 人の工程 | **検証用スペースを用意し API キーを発行する**（人しかできない。切り替えを試すなら 2 つ）。`backlog-facts.md` §5 の未確認 4・5・6・7・9・12・15 を実機で埋める（課題キーの先頭文字、小文字の URL、`/add/` のパス、`EditProject.action` の引数、カスタムドメインの形式、`Backlog-API-Key` ヘッダの CORS プリフライト）。スペース設定が admin 限定かを確認する（D-20 の未確認） |
| 依存 | M3 |
...
| 人の工程 | **計測の送信先と保持期間を決める**（決まるまで送信しない）。ストア提出。リポジトリを public にするなら `docs/reference` 相当の社内情報が無いことの確認 |
| 依存 | M6 |
...
| 5 | 検索は 1 スペースの中（D-20）。認証切れ・レート超過が行に閉じ、根では切り替え先のスペースが選べる | E2E |
