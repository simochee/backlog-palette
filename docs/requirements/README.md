...
| [`surfaces.md`](surfaces.md) | パレット以外の面（サイドパネル・ポップアップ・接続シート・設定画面・Firefox・カスタムドメイン・言語・計測） | container を書く人 |
| [`decisions.md`](decisions.md) | 要決定事項と推奨案。**着手前に埋める**。§0 に判断の前提（工数で縮小しない） | 判断する人 |
...
| 裁定 D4・D5（モーダル＝行く／サイドパネル＝探す） | **役割を変えて引き継ぐ** | 検索結果はパレットに出る（評価 §8.3）。サイドパネルは「詳しく見る面」になり、フィルターバーはそこに残る（[`decisions.md`](decisions.md) D-1・D-9） |
...
`⌘K` で出るパレットで、**ページ移動・課題キーのジャンプ・プロジェクトとスペースの切替・コピー・
スペース横断のキーワード検索（条件つき）**がキーボードだけで完結し、サイドパネルで結果の中身を開かずに読め、
Backlog 外からもポップアップで同じことができ、接続・設定・日英表示・Firefox・計測がそれを支える。
