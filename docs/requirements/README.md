...
| 裁定 D4・D5（モーダル＝行く／サイドパネル＝探す） | **役割を変えて引き継ぐ** | 検索結果はパレットに出る（評価 §8.3）。パレットは語とスコープだけ、サイドパネルは条件つきの詳細検索。フィルターバーはパネルに残る（[`decisions.md`](decisions.md) D-1・D-9） |
...
コピー・スペース横断のキーワード検索**がキーボードだけで完結し、条件を付けて探すときはサイドパネル（詳細検索）へ渡せる。
