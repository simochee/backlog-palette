...
### 10.0 既定は API キー（OAuth は任意）

**接続の既定を API キーにする。** OAuth は設定があるときだけ使える任意の経路として残す。

OAuth を既定にしていた唯一の理由は「キーのコピペが不要で非開発職にも通る」だった。その前提は、発行ページまで案内してコピペを 1 往復に収める導線（§10.2）で満たせる。一方、Backlog の OAuth には既定にするだけの利点が無い。

| 観点 | Backlog の OAuth | API キー |
|---|---|---|
| 権限の絞り込み | **できない**（認可リクエストに `scope` が無い） | できない |
| 失効 | アプリ一覧から | メモ付きで一覧から |
| 有効期限 | 3600 秒 + リフレッシュ | 無期限（更新処理が要らない） |
| 配布物の秘密 | `client_secret` が必須（PKCE 非対応） | **不要** |
| リダイレクト URI | ストア提出・Firefox 対応で登録し直し | 無関係 |

**権限を絞れないので、OAuth を使っても API キーと同じだけの権限を渡している。**最小権限という OAuth 本来の利点が存在しない以上、配布物に秘密を埋める代償とリダイレクト URI の維持コストに見合わない。Backlog が PKCE と scope に対応したら見直す。

### 10.2 API キーの接続導線

「キーを発行して、コピーして、拡張に戻って貼る」を「同じ画面で 1 回コピーして 1 回貼る」に縮める。

1. パレットの「このスペースを接続」で、そのスペースの `/EditApiSettings.action#bp-connect` へ遷移する
2. content script がフラグメントを見て、**Backlog のメモ欄に `Backlog Palette` を入力**しておく（後からどのキーが何用か分かるように）
3. あわせて**貼り付け先の拡張ページを iframe で出す**（パレットと同じ仕組み）
4. 発行するかどうか、どのキーを使うかは**利用者が選ぶ**。表示されたキーを利用者がコピーして、iframe の入力欄に貼る

**キーをページから自動で読み取らない。** 発行ページにはキーが平文で表示されているので技術的には読めるが、読み取る主体が拡張になると、content script が認証情報を扱う部品になり §2.3 の線が崩れる。読み取るのは人で、貼り付け先だけを拡張が用意する。

**貼り付け先をページ側の DOM に作らない。** 入力欄を content script が作ると、貼り付けたキーが Backlog のページのコンテキストを通る。拡張ページの iframe なら通らない。
...
|---|---|---|---|---|---|
| ~~M0~~ | スパイクと骨格 | 完了 | pnpm workspace、`packages/ui` + Storybook、`packages/core`、WXT アプリ（content + iframe + SW の最小実装） | ⌘K で iframe が出て入力欄にフォーカスが移り Esc で閉じる（未決 #6 #7 #14 解消）／Storybook が単体で起動し Row の Story が並ぶ | — |
| ~~M1~~ | Backlog API 検証 | 完了 | 検証スクリプトと計測レポート | `keyword` の一致範囲（課題 / Wiki / ドキュメント）とレートリミット実値が判明し、サイドパネル方針が確定（#1 #5） | — |
| ~~M2~~ | モーダル（API 非依存） | 完了 | スタックモデル、入力解釈、ローカル一致、ページ移動、コマンド、空状態、表示キャッシュ | A1〜A8 が実機で再現。課題キージャンプが `⌘K` + キー + `Enter` で完結。IME 完了条件を満たす | M0 |
| ~~M3~~ | 認証とスペース管理 | 完了（既定は API キー、§10.0） | OAuth 接続、API キー接続、C1・C2 | 未接続 → 接続 → 空状態に担当課題が出るまでが 1 分以内。失効時に行内から再接続できる | M1 |
| **M4** | サイドパネル検索 | 進行中（プレビューと履歴が残り） | 検索オーケストレーション、フィルターバー、プレビュー、ステータス帯、共有 URL、履歴 | B1〜B5 が実機で再現。1 スペース障害が他を妨げない。非同期追加で選択が動かない（D6 の合意を取る） | M2 M3 |
| ~~M5~~ | 個人化 | 完了 | frecency、クエリ辞書、履歴バックフィル、学習オフ / 履歴消去 | 介入ルール 4 件がテストで固定。`history` 未許可でも全機能が動く。消去で実際にデータが消える | M2 |
