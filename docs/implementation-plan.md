...
- **`⌫` がスタックと入力文字列のどちらに効くか**: モック A4-b は「入力した日本語は消えない」としか書いていないが、常にスタックに効かせると文字を消せなくなる。**キャレットが先頭（offset 0、選択なし）のときだけスタックに効かせる**。それ以外は通常の 1 文字削除。空入力時も offset 0 なので同じ規則で説明できる。この規則は実機で確認して合意を取る
...
| 17 | Phase 1 のページ移動のうち URL が未確定のもの | `core/nav` が作れない | お知らせ / 担当している課題 / 個人設定 / スペース設定 / メンバー / マイルストーン一覧 / 課題の追加。**スペースを開いて URL を控えるだけで埋まる** |
...
| 19 | Wiki 検索がプロジェクト指定必須である前提で §7.4 を組み直すか | スペース横断 Wiki 検索のコスト | `GET /api/v2/wikis` は `projectIdOrKey` が必須で、横断検索が参加プロジェクト数の並列になる。しかも本文が返らずスニペットも作れない。Wiki を横断検索の対象から外す判断もありうる |
...
実装に入る前に、**D4 の属性語検出の範囲**だけは決めてほしい（§18-13）。ここはコードの問題ではなく「どこまで賢くすると気持ち悪くならないか」の判断で、ユーザーの現場感が必要。

モック A8 は「処理中 決済」を「ステータス = 処理中」+ 語句「決済」に分解している。これを一般化するとき、選択肢は 3 つある。

| 案 | 検出する語 | 誤検出のリスク | 説明のしやすさ |
|---|---|---|---|
| **狭い** | そのスペースに実在するステータス名の**完全一致のみ**（未対応 / 処理中 / 処理済み / 完了 + カスタムステータス） | ほぼ無い | 「ステータス名を打つと条件になります」 |
| **中間** | 上記 + `自分の` `未完了` `期限切れ` の固定語彙 | 「自分の議事録」のような件名が壊れる | 語彙表を設定で見せる必要がある |
| **広い** | 上記 + ユーザー名・課題種別・マイルストーン名 | 高い。人名やマイルストーン名が普通の語と衝突する | 難しい |

「完了」はステータス名でありながら普通の日本語としても頻出する（「完了報告書」）ので、**狭い案でも `完了` だけは単独入力時に限る**などの例外が要る。

私の推奨は **狭い案から始める**（P1 と P6 を守れる。誤検出が 0 に近く、条件チップが出たときの理由が常に自明）。中間案の固定語彙は M2 のドッグフーディングで実際に打たれた入力を見てから足す。

この判断だけ決まれば、`src/core/query/detectConditions.ts` の仕様が書けて M2 に入れる。他の未決事項（#1 #6 #7 #10）は M0・M1 で私が潰す。
