...
function weightAt(at: number, now: number): number {
...
export function frecency(events: readonly ActivityEvent[], now: number): number {
  return events.reduce((total, event) => total + weightAt(event.at, now), 0);
}
...
export function frecencyByEntity(
  ...
  for (const event of events) {
    totals.set(event.entityId, (totals.get(event.entityId) ?? 0) + weightAt(event.at, now));
  }
  return totals;
}
