...
/** backlog-js のうち接続の手順が使う 3 つ。テストでは偽物を渡す */
...
export type ConnectDeps = {
  ...
  /** マスタ（プロジェクト・ステータス）を Query のキャッシュに先読みし、プロジェクト数を返す */
  prefetchMasters: (spaceHost: string) => Promise<{ projectCount: number }>;
  /** 同じ host があれば置き換える */
...
async function initialize(
  ...
  // 上限を知ってから先読みする。先読みは read 枠を 1 + プロジェクト数だけ使う
  await deps.initializeRateLimit(spaceHost, rateLimit);
  const { projectCount } = await deps.prefetchMasters(spaceHost);
...
/**
 * キーを検証してから保存し、上限とマスタ（プロジェクト・ステータス）を初期化する（surfaces.md §1.2）。
