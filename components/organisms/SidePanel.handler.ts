 ...
 * 行が出ていても、語を打ち直した後の Enter は検索の起動。行は前の語の結果で、打った語とは関係がない
 */
export function usePanelKeyHandler(props: SidePanelProps, listId: string) {
  return (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    const edited = props.searchedQuery !== undefined && value.trim() !== props.searchedQuery.trim();
