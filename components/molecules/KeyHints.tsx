...
export function fitHints(
  ...
  const byPriority = [...hints].sort((a, b) => a.priority - b.priority);
...
export function KeyHints({ hints }: KeyHintsProps) {
  ...
  useLayoutEffect(() => {
    ...
    if (!measure || available === undefined) return;

    const widths = new Map<string, number>();
    for (const child of measure.children) {
      const id = (child as HTMLElement).dataset.hintId;
      if (id) widths.set(id, child.getBoundingClientRect().width);
    }
